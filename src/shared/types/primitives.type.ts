...
export function StringWithLegthGen<N extends number>(value: string, n: N): StringWithLegth<N> {
	const fixed_length_logger = createChildLogger({
		fileType: 'core',
		service: 'typo',
		module: 'StringWithLegthGen',
	});

	if (typeof value !== 'string') {
		fixed_length_logger.error(
			{ expectedTypo: 'string', recivedTypo: typeof value },
			'Tipo de variavel errôneo',
		);
