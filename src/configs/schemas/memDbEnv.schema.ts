...
export const memEnvValidationSchema = z
	.object({
		...
		MEM_DB_SENTINEL_USERNAME: z.coerce
			.string({ error: 'O SentinelUsername deve ser uma string' })
			.optional()
			.refine(
				(val) => {
					if (!val) return true;
			...
			if (data.MEM_DB_SENTINEL_MASTER_ID) {
				if (!data.MEM_DB_SENTINEL_USERNAME) {
					ctx.addIssue({
						code: 'custom',
						message:
							'O MEM_DB_SENTINEL_USERNAME é obrigatório se o Sentinel estiver ativo em Produção ou Stage!',
				...
				if (!data.MEM_DB_SENTINEL_PASSWORD) {
					ctx.addIssue({
						code: 'custom',
						message:
							'O MEM_DB_SENTINEL_PASSWORD é obrigatório se o Sentinel estiver ativo em Produção ou Stage!',
