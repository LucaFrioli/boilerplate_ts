...
import { acceptedMemDatabaseProtocols } from '@Configs/Constants';
...
export class ValkeyConnectionString extends BaseMemUri {
	...
	private static readonly secParamSocket: string =
	...
	protected maskUriToLog(unmaskUri: string): string {
		...
		if (sentinelUser && sentinelUser !== '') {
			...
			if (sentinelUser && unmaskUri.includes('sentinelUsername=')) {
				unmaskUri = unmaskUri.replace(
					new RegExp(`sentinelUsername=${sentinelUser}`),
					`sentinelUsername=${maskLogDatabaseUsername(sentinelUser)}`,
				);
			}
		}
	...
	protected generateUriProd(validatedEnvValues: EnvDataForMemDbUri): MemDatabaseURI {
		...
		if (validatedEnvValues.MEM_DB_PROTOCOL === 'valkey+sentinel') {
			...
			if (!sentinelMasterId || sentinelMasterId === '') {
				this.handlerErrors({
					...
					message:
						'A variável de ambiente MEM_DB_SENTINEL_MASTER_ID é obrigatória para o protocolo valkey+sentinel',
				});
			}
			...
			if (
				validatedEnvValues.MEM_DB_SENTINEL_USERNAME &&
				validatedEnvValues.MEM_DB_SENTINEL_PASSWORD
			) {
		...
		try {
			if (this.candidateUri === '') {
				throw new Error(
					'Tentativa de realizar validação com uri inválida e vazia em produção',
					{
						cause: 'Erro na execução de criação da Uri possível tentativa de alteração da memória durante operação',
					},
				);
